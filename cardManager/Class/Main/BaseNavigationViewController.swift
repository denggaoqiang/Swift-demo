...
class BaseNavigationViewController: UINavigationController {
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        if viewController.navigationItem.leftBarButtonItem == nil && self.viewControllers.count > 0 {
            ...
            let backView = CustomNAVBackView.init(frame: CGRect(x: 0, y: 0, width: 65, height: 44))
            backView.navBackBtnBlock = {
                self.pop()
            }
            viewController.navigationItem.leftBarButtonItem = UIBarButtonItem.init(customView: backView)
        }
